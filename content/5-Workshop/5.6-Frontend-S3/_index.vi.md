...
> [!TIP]
> **Lưu ý quan trọng khi cập nhật giao diện (`index.html`):**
> 
> Nếu bạn đã tải tệp `index.html` mới lên Amazon S3 nhưng khi mở link CloudFront vẫn thấy giao diện cũ, đó là do **Amazon CloudFront** đang lưu bản ghi tạm (**Cache**). Để cập nhật giao diện mới ngay lập tức:
> 
> 1. **Tạo CloudFront Invalidation (Xóa cache trên CDN):**
>    - Mở **Amazon CloudFront Console** ➔ Chọn Distribution của bạn.
>    - Chuyển sang tab **Invalidations** ➔ Nhấn **Create invalidation**.
>    - Tại ô **Object paths**, nhập `/*` (hoặc `/index.html`) ➔ Nhấn **Create invalidation**.
> 2. **Xóa Cache Trình duyệt (Hard Refresh):**
>    - Nhấn **`Ctrl + F5`** (hoặc `Ctrl + Shift + R` / `Cmd + Shift + R`) hoặc mở bằng **Cửa sổ ẩn danh (Incognito)** để tải lại bản mới nhất.
